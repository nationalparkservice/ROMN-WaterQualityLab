#' Obtain metadata from the SEI database used to format our EDD
#'
#' @param file_path File path to a copy of the SEI database.
...
#'
#' @return A list containing the three tables required from the database.
load_db <- function(file_path, pre_2020 = FALSE) {
  ...
  dbo_Site <- DBI::dbReadTable(SEI_DB,"dbo_Site")
  dbo_SitesMetadata <- DBI::dbReadTable(SEI_DB,"dbo_SitesMetadata")
  ...
  if (is.character(pre_2020)) {
    ...
    dbo_Joined <- read_excel(pre_2020, na = "ND") %>%
    ...
    dbo_Joined <- dbo_CollectionEvent %>%
      left_join(dbo_WaterSediment, by = c("ID" = "CollectionEventID")) %>%
      select(ID, SampleNumber, EventName, Notes) #%>% # CODE FOR TEST FOR BILLY
      # mutate(EventName = case_when(EventName == "GLAC_S04_20230823" ~ "GLAC_S04_20230823_FB_W",
      #                              EventName == "GLAC_S04_20230823_FB_W" ~ "GLAC_S04_20230823",
      #                              TRUE ~ EventName))
  }
  return(list("Site" = dbo_Site, "SitesMetadata" = dbo_SitesMetadata, "Joined" = dbo_Joined))
}
...
#' @param db_file_path File path to a copy of the SEI database.
#' @param pre_2020 By default set to false. If you are using data from before 2020, instead set this 
#' to a file path to ROMN_SEI_in_EQuIS.xlsx, which includes a crosswalk between EventName and SampleNumber.
...
format_edd_ccal <- function(ccal_file_paths, db_file_path, pre_2020 = FALSE, 
                            limits = imdccal::limits, qualifiers = imdccal::qualifiers, concat = FALSE){
  ...
  package_data <- getCCALData(ccal_file_paths, concat) %>%
    lapply(function(x) {
      x[[1]] %>%
        handle_duplicates() %>%
        dplyr::left_join(limits %>% dplyr::select(-analysis), # join MDL and ML to data
  ...
  dbo <- load_db(db_file_path, pre_2020)
  ...
  # ------------------------
  edd_results <- pmap(list(format_results(ccal_file_paths, limits = limits, qualifiers = qualifiers, concat = concat), 
  ...
  SEI_DB <- DBI::dbConnect(odbc::odbc(), .connection_string = paste0("Driver={Microsoft Access Driver (*.mdb, *.accdb)};Dbq=", 
                                                                     db_file_path))
  edd_projects <- DBI::dbReadTable(SEI_DB, "tbl_Projects_EDD") %>%
...
#' @param limits Table with detection limits. By default, uses the version in the imdccal package. User-defined versions must have the same columns.
#' @param qualifiers Table with flags and their meanings. By default, uses the version in the imdccal package. User-defined versions must have the same columns.
#' @param concat If concat is set to TRUE, the function creates one file rather than one for every input file.
...
#'           overwrite = TRUE)
#' }
write_edd <- function(ccal_file_paths, db_file_path, pre_2020, format = c("xlsx", "csv"), destination_folder = "./", overwrite = FALSE,
                      limits = imdccal::limits, qualifiers = imdccal::qualifiers, concat = FALSE) {
  format <- match.arg(format)
  destination_folder <- normalizePath(destination_folder, winslash = .Platform$file.sep)
  
  # Call function to format EDD
  all_data <- format_edd_ccal(ccal_file_paths, db_file_path, pre_2020, limits, qualifiers, concat)
